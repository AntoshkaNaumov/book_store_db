...
# запросы
# Выводит издателя (publisher), имя которого принимается через input()
q = session.query(Publisher).filter(Publisher.name == input("Введите название издателя "))
for s in q.all():
    print(s.id, s.name)

# Выводит издателя (publisher), идентификатор которого принимается через input()
q = session.query(Publisher).filter(Publisher.id == input("Введите идентификатор (id) издателя "))
for s in q.all():
    print(s.id, s.name)

# выводит магазины с наименованием и его id
subq = session.query(Shop).all()
for s in subq:
    print(s.id, s.name)

# запрос выборки магазинов, продающих целевого издателя
print(session.query(Shop.name, Shop.id).join(Stock).join(Book).join(Publisher).filter(Publisher.name == input("Введите название издателя ")).all())
